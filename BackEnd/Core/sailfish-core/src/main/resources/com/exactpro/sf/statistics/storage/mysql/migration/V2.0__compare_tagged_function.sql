...
FROM 
 (SELECT TC.id AS tcid, TC.testcaseid, M.name, TCR.status, TCR.failreason, TCR.comment, TS.name AS userStatus, TCR.id AS tcrid, MR.id AS mrid,
        ...
        (SELECT GROUP_CONCAT(CONCAT(AR.rank, ':::', AR.failreason) SEPARATOR ';;;')  FROM stactionruns AR WHERE AR.tc_run_id = TCR.id AND AR.failreason IS NOT NULL) AS failedactions
    FROM 
