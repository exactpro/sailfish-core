...
WITH first_tcrs AS (
    ...
    SELECT TC.id AS tcid, TC.testcaseid, M.name, TCR.status, TCR.failreason, TCR.comment, TS.name AS userStatus, TCR.id AS tcrid, MR.id AS mrid,
        ...
        (SELECT string_agg(AR.rank::text || ':::' || AR.failreason, ';;;' ORDER BY AR.rank)  FROM stactionruns AR WHERE AR.tc_run_id = TCR.id AND AR.failreason IS NOT NULL) AS failedactions
    FROM 
