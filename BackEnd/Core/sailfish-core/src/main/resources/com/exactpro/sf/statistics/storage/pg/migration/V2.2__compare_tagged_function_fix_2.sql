...
-- function body is the same is in V2.1, but splitters changed
-- see StatisticsReportingStorage.ACTION_RANK_FREASON_SPLITTER, StatisticsReportingStorage.FAILED_ACTION_SPLITTER
CREATE OR REPLACE FUNCTION tagged_sets_comparison(IN first_ids bigint[], IN first_count bigint, IN second_ids bigint[], IN second_count bigint)
    ...
    ftcid bigint, fmatrix character varying(255), fstatus integer, ffailreason character varying(255), ffailedactions text, fcomment text, fuserStatus text, ftcrid bigint, fmrid bigint,
        fstarttime timestamp, ffinishtime timestamp, ftags text, fhash integer,
    stcid bigint, smatrix character varying(255), sstatus integer, sfailreason character varying(255), sfailedactions text, scomment text, suserStatus text, stcrid bigint, smrid bigint,
...
SELECT coalesce(F.testcaseid, S.testcaseid) AS testcaseid,
    F.tcid AS ftcid, F.name AS fmatrix, F.status AS fstatus, F.failreason AS ffailreason, F.failedactions AS ffailedactions, F.comment AS fcomment, F.userStatus AS fuserStatus, F.tcrid AS ftcrid, F.mrid AS fmrid,
        F.starttime AS fstarttime, F.finishtime AS ffinishtime, F.tags AS ftags, F.hash AS fhash,
    S.tcid AS stcid, S.name AS smatrix, S.status AS sstatus, S.failreason AS sfailreason, S.failedactions AS sfailedactions, S.comment AS scomment, S.userStatus AS suserStatus, S.tcrid AS stcrid, S.mrid AS smrid,
