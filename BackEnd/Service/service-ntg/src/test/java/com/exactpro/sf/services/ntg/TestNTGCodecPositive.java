...
public class TestNTGCodecPositive extends AbstractTest {
	...
	public void testDecodeOrderRejectFromBytes(){
		int[] ints = {
                ...
                0x41, 0x6c, 0x41, 0x5f, 0x34, 0x00, 0x37, 0x31, 0x34, 0x34, 0x38, 0x37, 0x33, 0x32, 0x30, 0x31,
                0x39, 0x31, 0x32, 0x33, 0x31, 0x2d, 0x31, 0x31, 0x3a, 0x32, 0x32, 0x3a, 0x33, 0x33, 0x2e, 0x31,
                0x32, 0x33, 0x34, 0x35, 0x36, 0x00, 0x67, 0x8b, 0xfa, 0x4b, 0x18, 0xd8, 0x05, 0x00, 0x00
		};
		try{
			decodeBytes(ints);
		}catch(Exception e){
	...
	public void testDecodeExecutionReportFromBytes()
	{
		int[] ints = {
                ...
                0x00, 0x00, 0x65, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66,
	...
	public void testDecodeCancelRejectFromBytes()
	{
		int[] ints = {
                ...
                0x45, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xd0, 0x07, 0x00, 0x00, 0x32, 0x30, 0x31,
                0x39, 0x31, 0x32, 0x33, 0x31, 0x2d, 0x31, 0x31, 0x3a, 0x32, 0x32, 0x3a, 0x33, 0x33, 0x2e, 0x31,
	...
	public void testEncodeDecodeMessageWithDateType() {
		...
		message.addField("MessageHeader", messageHeader);
        DateTimeFormatter dateTimeFormatter = DateTimeUtility.createFormatter("yyyyMMdd-hh:mm:ss.SSSSSS");
        String dateTimeStr = "20191231-11:22:33.123456";
        TemporalAccessor dateTime = dateTimeFormatter.parse(dateTimeStr);
		message.addField("testDate", DateTimeUtility.toLocalDateTime(dateTime));
		try{
            testRoundTrip(message, TestNTGHelper.getDictionaryWithDifferentTypesMessages());
		}catch(Exception e){
			logger.error(e.getMessage(),e);
	...
	private void testRoundTrip(IMessage message, IDictionaryStructure dictionary) {
		try
		{
			...
			if (lastMessage instanceof IoBuffer) {
                ...
                IoBuffer srcBuf = (IoBuffer)lastMessage;
                toDecode.put(srcBuf);
				...
				decodeCodec.decode(decodeSession, toDecode, decoderOutput);
	...
	private void decodeBytes(int[] ints) throws Exception{
		...
		Assert.assertTrue( "Test for decoding error.", decodableResult);
        AbstractProtocolDecoderOutput decoderOutput2 = new MockProtocolDecoderOutput();

		decodeCodec2.decode( decodeSession2, toDecode, decoderOutput2 );

        Assert.assertTrue("Message queue size must not less then 1.", decoderOutput2.getMessageQueue().size() >= 1);
		Object decodedMessage2 = decoderOutput2.getMessageQueue().element();
