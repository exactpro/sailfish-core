...
(function(root, factory) {
    ...
    QueryBuilder.prototype.bindEvents = function() {
        ...
        this.$el.on('change.queryBuilder', '.rules-group-header [name$=_cond]', function() {
            ...
            if ($this.is(':checked')) {
                $this.parent().addClass('active-and-or').siblings().removeClass('active-and-or');
            }
        });
...
<dl id="'+ group_id +'" class="rules-group-container"> \
  <dt class="rules-group-header"> \
    <div class="btn-group pull-right group-actions"> \
      <button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only add-rule-button" data-add="rule"> \
        <i class="' + this.icons.add_rule + '"></i> '+ this.lang.add_rule +' \
      </button> \
    '+ (this.settings.allow_groups===-1 || this.settings.allow_groups>=level ?
      '<button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only add-group-button" data-add="group"> \
        <i class="' + this.icons.add_group + '"></i> '+ this.lang.add_group +' \
      </button>'
    :'') +' \
    '+ (level>1 ? 
      '<a type="button" class="delete-group-button ui-menuitem-icon ui-icon eps-custom-icon-close eps-custom-icon" data-delete="group"></a>'
    : '') +' \
    </div> \
  ...
  <dd class=rules-group-body> \
    ...
    QueryBuilder.prototype.getGroupConditions = function(group_id) {
        ...
        for (var i=0, l=this.settings.conditions.length; i<l; i++) {
            ...
            h+= '\
            <label class="and-or-button '+ (active?'active-and-or':'') +'"> \
              <input type="radio" name="'+ group_id +'_cond" value="'+ cond +'" '+ (active?'checked':'') +'>'+ label.trim() +'</label>';
        }
...
<li id="'+ rule_id +'" class="rule-container"> \
  <div class="rule-header"> \
    <div class="btn-group pull-right rule-actions"> \
      <a type="button" class="delete-group-button ui-menuitem-icon ui-icon eps-custom-icon-close eps-custom-icon" data-delete="rule"></a> \
    </div> \
  </div> \
  '+ (this.settings.display_errors ?
...
$.fn.queryBuilder.define('sortable', function(options) {
        ...
        this.on('afterInit', function() {
            ...
            this.$el.on('mouseover', '.rule-container, .rules-group-container', function() {
                that.$el.find('.rule-container, .rules-group-container').attr('draggable', true);
            });
            this.$el.on('mouseout', '.rule-container, .rules-group-container', function() {
        ...
        this.on('afterApplyRuleFlags', function($rule, rule, flags) {
            if (flags.no_sortable) {
                $rule.find('.rule-container').remove();
                $rule.find('.rules-group-container').remove();
            }
        });
