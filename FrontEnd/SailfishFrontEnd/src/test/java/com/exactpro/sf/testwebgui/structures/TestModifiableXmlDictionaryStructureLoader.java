...
public class TestModifiableXmlDictionaryStructureLoader {
    private final Path BASE_DIR = Paths.get((System.getProperty("basedir") == null) ? "." : System.getProperty("basedir")).toAbsolutePath().normalize();
    private final String path = BASE_DIR + File.separator + "src" + File.separator + "test" + File.separator + "resources" + File.separator;
    ...
    public void testMessageInheritance() throws Exception {
        try (InputStream stream = new FileInputStream(path + "message-inheritance.xml")) {
            Map<String, IMessageStructure> messages = new ModifiableXmlDictionaryStructureLoader().load(stream).getMessages();
