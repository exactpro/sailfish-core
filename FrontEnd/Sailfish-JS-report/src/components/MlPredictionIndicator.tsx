...
export class MessagePredictionIndicatorBase extends Component<MessagePredictionIndicatorProps, {}> {

    render({ activeActionId, messageId, className, predictions, predictionsEnabled }: MessagePredictionIndicatorProps) {
        if (!predictionsEnabled) {
            return null
        }

        const predictedValue = predictions
            .find((prediction) => { return prediction.messageId === messageId && prediction.actionId === activeActionId; })

        if (predictedValue) {
            return (
                <div
                    class={className}
                    style={`background-color: hsl(${COLOR_HUE}, ${COLOR_SATURATION_PERCENTAGE}%, ${getLightness(predictedValue.predictedClassProbability)}%)`}>
                    <div class="ml__prediction-icon" />
                </div>
            )
        }
    }
